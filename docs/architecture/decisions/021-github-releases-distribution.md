...
- Linux `.deb` / `.tar.gz`. The `.deb` installs `/usr/share/icons/hicolor/256x256/apps/ayutam.png` so `Icon=ayutam` in the desktop entry resolves, and depends on `libayatana-appindicator3-1` (the tray library `tray_manager` links and does not bundle). The portable tarball does not include that library. CI installs `libayatana-appindicator3-dev` before `flutter build linux`. Package description covers shipped phases including portable `.skilltracker` backups.
