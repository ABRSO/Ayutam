...
- **Android artifacts:** `flutter build apk --release --split-per-abi` (arm64-v8a / armeabi-v7a / x86_64). Compile mode is **release** (AOT). Signing uses the permanent Ayutam release keystore (`android/key.properties` locally; repository secrets in CI). Never commit the keystore or passwords. Play upload keystore / AAB remain later work.
