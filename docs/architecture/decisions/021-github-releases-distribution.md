...
- Linux `.deb` / `.tar.gz`. The `.deb` installs `/usr/share/icons/hicolor/256x256/apps/ayutam.png` so `Icon=ayutam` in the desktop entry resolves; package description covers shipped Phases 0–5 including portable `.skilltracker` backups.
