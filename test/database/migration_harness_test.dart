...
/// Migration harness for schema v1 fixtures (Phase 5 task 9).
///
/// Builds an on-disk schema-v1 database (no FTS), then opens it with the
/// current [AppDatabase] so future schema bumps can add assertions here.
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('ayutam_mig_harness_');
  ...
  test('schema v1 fixture migrates to current schema with data intact', () async {
...
''');
    ...
    raw.execute('INSERT INTO schema_metadata (key, value) VALUES (?, ?)', [
      'data_format_version',
      '1',
    ]);
    raw.execute(
    ...
    raw.execute(
      'INSERT INTO skills (id, name, created_local_date, created_at_utc, '
      'updated_at_utc, source_device_id) VALUES (?, ?, ?, ?, ?, ?)',
      [skillId, 'Harness', '2026-01-01', now, now, deviceId],
    );
    raw.execute(
      'INSERT INTO sessions (id, skill_id, status, start_at_utc, end_at_utc, '
      'active_seconds, timezone_id_at_creation, offset_minutes_at_start, '
      'created_at_utc, updated_at_utc, source_device_id) '
      "VALUES (?, ?, 'completed', ?, ?, 60, 'UTC', 0, ?, ?, ?)",
      [sessionId, skillId, now - 60000, now, now, now, deviceId],
    );
    raw.close();
    ...
    await indexer.ensureCreated();
    final fts = await db
        .customSelect('SELECT count(*) AS c FROM session_search')
        .getSingle();
    expect(fts.read<int>('c'), greaterThanOrEqualTo(0));
